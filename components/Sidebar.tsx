...
const Sidebar: React.FC = () => {
  ...
  const navItems = [
    { label: 'Boards', path: '/app/boards', icon: Trello },
    { label: 'Pages', path: '/app/pages', icon: FileText },
    { label: 'Membros', path: '/app/settings/members', icon: Users },
  ];
          ...
          {navItems.map((item) => (
            ...
            >
              <item.icon className={cn(
                "w-4 h-4",
                location.pathname.startsWith(item.path) ? "text-white" : "text-slate-500 group-hover:text-slate-300"
              )} />
              {item.label}
            </Link>
          ))}
